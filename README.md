...
Beating a level unlocks the next, and from then on Start Game opens a level select. Every level is
flown from a standing start: score, experience and power-ups reset when it begins. The art for both
is generated by the scripts in `tools/`.
